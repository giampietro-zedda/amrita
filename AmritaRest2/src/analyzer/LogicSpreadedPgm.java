...
public class LogicSpreadedPgm extends ExecutionShared implements AmritaConstants {
	 ...
	 *  
	 */
	public LogicSpreadedPgm(UserConfiguration sd, ExecutionDirectives di, ProgramCobol program) {
		super(sd, di);
	...
	public ArrayList<String> dynamicValues(
		...
		ArrayList<String> al_pgmNameCallerOrigin = null;            // Programmi chiamanti programma origine Ottimizzazione X ottimizzazione
		ArrayList<String> al_pgmNameCaller = null;                  // Programmi chiamanti ultima assegnazione da risolvere
		...
		ilpw.ucfg = this.ucfg;                                      // Per accesso dati in metodi statici
        ...
        for (LogicDynamicField logicDynamicField : ilpw.logicDynamicInstructionOrigin.al_dynamicField) {
			if (logicDynamicField.dataItemCobolIdentifier.getNameIdentifier().equals(ilpw.dynamicFieldName)) {
				...
				ilpw.dynamicFieldToSolve = logicDynamicField;
				break;
			}
		}
		...
		// Get nomi programmi chiamanti, che chiamano direttamente il programma  origine
        al_pgmNameCallerOrigin = LogicTools.getPgmCallers(programOrigin.getProgramName(), di.systemInput, di.subSystemInput, ucfg);			 
		...
		for (LogicDynamicFieldSub logicDynamicFieldSub : ilpw.al_DynamicFieldSubWrk) {
            ...
            if (logicDynamicFieldSub.isSolved && !logicDynamicFieldSub.isWaitingForExternalData) {
				continue;
			}

            // Scan ultime assegnazioni sottocampo generate a partire dal programma ORIGINE
            // Possono essere presenti anche solo assegnazioni parziali ancora da risolvere.
            // al_lastSetTotal è stato valorizzato a livello di LogicSamePgm
			for (LogicDynamicFieldSubSetting lastSetTotal : logicDynamicFieldSub.al_lastSetTotal) {

				ilpw.lastSetSpreaded = lastSetTotal;     // Ultima assegnazione da risolvere per il sottocampo in programma chiamante o per valori esterni
				...
				// Ultima assegnazione ancora da risolvere, in quanto in attesa di dati esterni 
				// o parametro di call using o campo di dfhcommarea/linkage/twa/csa
				
				// Ultima assegnazione da risolvere nei programmi chiamanti
				if (isLastSetToBeSolvedInCaller(lastSetTotal)) {
					
					// Ultima assegnazione fatta nel programma origine
					if (lastSetTotal.entityDynamicFieldSetting.getIdPgmSet().equals(lastSetTotal.entityDynamicFieldSetting.getIdObject())) {
						al_pgmNameCaller = al_pgmNameCallerOrigin;		
					// Ultima assegnazione in un programma chiamante
					} else {
						al_pgmNameCaller = LogicTools.getPgmCallers(lastSetTotal.entityDynamicFieldSetting.getIdPgmSet(), di.systemInput, di.subSystemInput, ucfg);								
					}
					
					// Utilizzato nel processo di estrazione
					ilpw.logicDynamicFieldSubOriginLastSet = lastSetTotal;
					
					// Soluzione ultima assegnazione sottocampo (o una sua porzione) nei programmi chiamanti.
					// Una volta risolta l'assegnazione, viene popolata le strutture del programma origine con i valori individuati.
				...
				// Ultima assegnazione in attesa di dati esterni
				if (isLastSetWaitingForData(lastSetTotal)) {
					
					// Utilizzato nel processo di estrazione
					ilpw.logicDynamicFieldSubOriginLastSet = lastSetTotal;					
			        ilpw.programCur = ilpw.programOrigin;
			        ilpw.curLastSet = lastSetTotal.entityDynamicFieldSetting.getSetMode();
			        	
					// Estrazione valori da db se presenti
					// Valori inseriti nella struttura del sottocampo se individuati
					ilpw.al_valuesField = this.logicSamePgm.getLogicTools().lastSetByCicsReadWaitingForData(ilpw, lastSetTotal, this.analyzerDbInfo);
		...
		// Generazione valori campi in input come Composizione valori sottocampi 
		// In caso di campo elementare senza assegnazioni parziali frammentate 
		// NON è necessario ricomporre i valori del sottocampo 
		ilpw.dynamicFieldToSolve = ilpw.logicDynamicFieldOrigin;    // Dati campo dinamico origine per calcolo valori finali  
		LogicTools.generateFieldValuesBySubFieldsComposition(ilpw);
		
		// Nessun nuovo valore individuato nei programmi chiamanti/in attesa di dati esterni
		if (ilpw.dynamicFieldToSolve == null) {
			return ilpw.al_valuesField;
		}
		
		// I valori estratti nei programmi chiamanti o da dati esterni sono stati accodati 
		// a livello di sottocampo e combinati a livello di campo,
		...
		LogicTools.insertFullFieldValues(ilpw, ilpw.dynamicFieldToSolve.al_valuesField);
	 ...
	 * ---------------------------------------------------------------------
	 * 
	 * Si trattano i programmi chiamanti del programma di ultima assegnazione 
	 ...
	 * che imposta i valori e quindi a risolvere l'istruzione dinamica
	 */
	private void dynamicValuesSpreadedExtractor( LogicWorkProcess ilpw						// Stato ricorsivo iniziale sul programma origine o un suo chiamante
        ...
        al_valuesInProgramCaller = new ArrayList<String>();                 // Valori cumulativoi individuati in callers allo stesso livello
		...
		for (String pgmNameCaller : al_pgmNameCaller) {
			...
			for (LogicCallerOrigin logicCallerOrigin : al_callerOrigin) {
				...
				// Individuazione nuovo campo di cui trovare i valori, nel programma chiamante corrente (chiamante)
				...
				// Nel chiamante potrebbero esserci assegnazioni multiple a pointers che indirizzano aree diverse.
				// Pertanto potrebbero essere individuati più campi nel chiamante di cui cercare i valori.
				// Al momento si considera solo il primo campo individuato in una specifica area di working.
				...
				al_valuesInProgramCaller.addAll(ilpw.al_valuesField);
		...
		// Il chiamante si aspetta i valori in ilpw.al_valuesField come per LogicSamePgm
		ilpw.al_valuesField = al_valuesInProgramCaller;
	}
    ...
    */
	...
	private ArrayList<DataItemCobolIdentifier> detectNewFieldsOnProgramCaller(LogicWorkProcess ilpw) {
		...
		// Informazioni sul campo da cercare nel chiamante, a fronte di passaggio parametri in cics Twa
   		...
   		// Informazioni sul campo da cercare nel chiamante, a fronte di generica area di linkage indirizzata da pointers in Using|Dfhcommarea|TWA|CSA
     ...
     * Si restituisce l'identificatore del campo individuato.
     * 
     */
     ...
     * 
     * ptr può essere definito direttamente in Using|DFCOMMAREA|TWA|CSA oppure essere assegnato via 
     * 
     *   MOVE ptr-redefined TO ptr-redefined in Using|DFCOMMAREA|TWA|CSA
     * 
     * 1) Si recupera il nome del pointer da cercare da Using|DFCOMMAREA|TWA|CSA
     * 2) Si individua l'area di working cercando
     *   SET ptr TO ADDRESS OF area-working
     * 3) Si individua, se esiste, il campi alla posizione richiesta
     * 
     * Ottimizzazione
     * 
	...
	private ArrayList<DataItemCobolIdentifier> getNewFieldOnProgramCallerByLinkage(LogicWorkProcess ilpw) {
		...
		ArrayList<DataItemCobolIdentifier> al_areaWorkingAddressed = null;
		ArrayList<InstructionCobolDataItem> al_areaWithPointer = null;
		ArrayList<DataItemCobolIdentifier> al_newFieldFound = null;
		ArrayList<DataItemCobolIdentifier> al_newFieldFoundWork= null;
		ArrayList<InstructionCobolDataItem> al_dataItemPointer = null;
		...
		al_areaWorkingAddressed = new ArrayList<DataItemCobolIdentifier> ();
		...
		if (ilpw.lastSetSpreaded.entityDynamicFieldSetting.getTypePointerArea() == EnumLogicSetPointerArea.POINTER_INSIDE_USING_PARM) {
			al_areaWithPointer = getAreaOfUsingParm(ilpw, ilpw.callerOrigin.numInstr, ilpw.lastSetSpreaded.entityDynamicFieldSetting.getNumUsingParmPointer());
			if (al_areaWithPointer.size() == 0) {
				return null;
			}
		...
		// POINTER_INSIDE_CICS_DFHCOMMAREA
			...
			al_areaWithPointer = getAreasCicsDfhcommarea(ilpw);
			if (al_areaWithPointer.size() == 0) {
				return null;
			}
		...
		// POINTER_INSIDE_CICS_TWA
			...
			al_areaWithPointer = getAreasCicsTwaCsa(ilpw, "TWA");
			if (al_areaWithPointer.size() == 0) {
				return null;
			}
		...
		// POINTER_INSIDE_CICS_CSA
			...
			al_areaWithPointer = getAreasCicsTwaCsa(ilpw, "CSA");
			if (al_areaWithPointer.size() == 0) {
				return null;
		    }
     	}
		
		areaPointerPos = ilpw.lastSetSpreaded.entityDynamicFieldSetting.getDspPointerInLinkageArea() + 1; // Per chiarezza
					
		
		/////////////////////////////////////////////////////////////////////////////////////////////////
		// (2) Individuazione pointer in aree individuate alla posizione di ultima assegnazione
		//     Le aree supplementari sono dovute a eventuali redefines del parametro di using 
		/////////////////////////////////////////////////////////////////////////////////////////////////

		al_dataItemPointer = new ArrayList<InstructionCobolDataItem>();
		
		// Scan Aree candidate (livelli 01)
		for (InstructionCobolDataItem areaWithPointer : al_areaWithPointer) {
			dataItemPointer = getPointerOnArea(ilpw, areaWithPointer, areaPointerPos);
			
			// Pointer individuato
			if (dataItemPointer != null) {
				al_dataItemPointer.add(dataItemPointer);
			}
		}
				
		////////////////////////////////////////////////////////////////////////////
		// (3) Estrazione aree indirizzate dai pointers individuati
		////////////////////////////////////////////////////////////////////////////
		
		// Scan istruzioni SET pointer TO ADDRESS OF Area presenti nel programma
		...
		for (int numInstrAddress : ilpw.set_setCobolToAddressOf) {
			...
			// Scan pointers individuati nel parametro using e nei suoi redefines 
			for (InstructionCobolDataItem pointer : al_dataItemPointer) {
				
				// E' il pointer che interessa
				if (!identifierPointer.getDataItem().getDataName().equals(pointer.getDataName())) {
					continue;
				}
			...
			al_areaWorkingAddressed.add(identifierAreaAddressed);
		...
		if (al_areaWorkingAddressed.size() == 0) {
			return al_newFieldFound;
		}
	 
		////////////////////////////////////////////////////////////////////////////
		// (3) Individuazione di tutti campi validi in aree individuate
		////////////////////////////////////////////////////////////////////////////
		...
		for (DataItemCobolIdentifier areaWorking : al_areaWorkingAddressed) {
	...
	/* ----------------------------------------------------------------------------
	 * Restituzione del pointer trovato nelle aree candidate al displacement fornito
	 * ----------------------------------------------------------------------------
	 * 
	 * Per ricerca valori a fronte di Linkage con pointer in using parm.
	 * 
	 * Nell'area fornite in input, che e' il livello 01 del parametro in call using
	 * o una sua redefines si individua il pointer e si estrare il suo nome.
	 * Si restituisce il pointer che inizia al displacement fornito in input.
	 * 
	 * Il nome del pointer viene poi utilizzato per individuare l'area di working storage
	 * dove si trova il campo di cui cercare le assegnazioni.
	 * 
	 * L'area di working si individuera con:
	 * 
	 *   SET pointer-name TO ADDRESS OF area-working
	 * 
	 * Si restituisce l'identificatore del pointer trovato
	 * 
	 */
    private InstructionCobolDataItem getPointerOnArea(LogicWorkProcess ilpw
										    	   , InstructionCobolDataItem areaWithPointer
												   , int dspInArea) {
    	
		ProgramCobolEntry<? extends Instruction> entryData = null;				// Entry data division
		ProgramCobolEntry<? extends Instruction> entryArea = null;		        // Entry data division
		InstructionCobolDataItem dataItemGeneric1 = null;						// Data item generico di servizio
		InstructionCobolDataItem dataItemGeneric2 = null;						// Data item generico di servizio
		InstructionCobolDataItem dataItemGeneric3 = null;						// Data item generico di servizio
		int dspCur = 0;                                                         // Displacement corrente campo in area
		int dspArea = 0;                                                        // Displacement area
		int lvlArea = 0;                                                        // Numero livello area
		int lvlFieldAtDsp = 0;                                                  // Numero livello campo al displacement fornito

		// Campo di gruppo: ricerca sottocampo compatibile con la posizione richiesta
		entryArea = ilpw.callerOrigin.programCaller.entriesData()[areaWithPointer.getNumInstr()];
		lvlArea = areaWithPointer.getLevelNumber();
		dspArea = entryArea.getPos();                 // 0-based
		
        // Cerco il pointer che inizia al displacement previsto, nel parametro using passato dal chiamante al chiamato
 		
 		// Scan campi sotto area (inclusa)
 		for (int i = areaWithPointer.getNumInstr() + 1; i < ilpw.callerOrigin.programCaller.entriesData().length; i++) {
	        ...
	        dataItemGeneric1 = (InstructionCobolDataItem) entryData.getInstruction();

			// Considero solo le istruzioni dichiarative
			if (dataItemGeneric1.getLevelNumber() == 88
		    ||  dataItemGeneric1.getLevelNumber() == 66) {
				continue;
			}

			// Altro livello uguale a quello dell'area in cui cercare.
			// Sicuramente Pointer non individuato al displacement fornito
			if (dataItemGeneric1.getLevelNumber() <=  lvlArea ) {
				break;
			}
			...
			// Continua ricerca
			if (dspCur < dspInArea) {
				dspCur += dataItemGeneric1.getSizeBytes();
				continue;
			}
			
			// Campo sicuramente non presente
			// Aree disallineate fra chiamante e chiamato
			if (dspCur > dspInArea) {
				break;
			}
			
			// Displacement coincidente: verifica diverse situazioni di definizione del pointer
			
			// Pointer trovato direttamente al displacement fornito
			// Si cerca situazione
			//   05 PTR PTR-ADDR USAGE POINTER.
			if (dataItemGeneric1.isUsageClause() 
			&&  dataItemGeneric1.getUsage() == EnumCobolUsage.USAGE_POINTER) {
				return dataItemGeneric1;
			}

			// Trovato Campo di gruppo 05 PTR-X. al displacement fornito
			if (dataItemGeneric1.isGroupField()) {
				lvlFieldAtDsp = dataItemGeneric1.getLevelNumber();
				if ((i + 1) < ilpw.callerOrigin.programCaller.entriesData().length) {
					entryData = ilpw.callerOrigin.programCaller.entryDataDivision(i + 1);
					dataItemGeneric2 = (InstructionCobolDataItem) entryData.getInstruction();
				}
				if ((i + 2) < ilpw.callerOrigin.programCaller.entriesData().length) {
					entryData = ilpw.callerOrigin.programCaller.entryDataDivision(i + 2);
					dataItemGeneric3 = (InstructionCobolDataItem) entryData.getInstruction();
				}
				
				// Si cerca situazione
				//   05 PTR-X.
				//      07 PTR-ADDR PIC S9(8) COMP. 
		        //      07 PTR REDEFINES PTR-ADDR USAGE POINTER.
				if (dataItemGeneric2.getLevelNumber() > lvlFieldAtDsp
				&&  dataItemGeneric2.getSizeBytes() == 4                                 // 9(8)
				&& 	dataItemGeneric2.getItemType() == EnumDataItemType.COBOL_BINARY      // COMP
				&&  dataItemGeneric3.isRenamesClause()
				&&  dataItemGeneric3.getLevelNumber() == dataItemGeneric2.getLevelNumber()
				&&  dataItemGeneric3.getRedefinesDataName().equals(dataItemGeneric2.getDataName())
				&&  dataItemGeneric3.getUsage() == EnumCobolUsage.USAGE_POINTER) {
					return dataItemGeneric3;
				}

				// Si cerca situazione
				//   05 PTR-X.
				//      07 PTR-ADDR PIC S9(8) COMP.
				//   05 PTR REDEFINES PTR-X USAGE POINTER.
				if (dataItemGeneric2.getLevelNumber() >  lvlFieldAtDsp
				&&	dataItemGeneric3.getLevelNumber() == lvlFieldAtDsp
				&&  dataItemGeneric2.getSizeBytes() == 4                                 // 9(8)
				&& 	dataItemGeneric2.getItemType() == EnumDataItemType.COBOL_BINARY      // COMP
				&&  dataItemGeneric3.isRedefinesClause()
				&&  dataItemGeneric3.getRedefinesDataName().equals(dataItemGeneric1.getDataName())
				&&  dataItemGeneric3.getUsage() == EnumCobolUsage.USAGE_POINTER) {
					return dataItemGeneric3;
				}

				// Si cerca situazione
				//   05 PTR-ADDR PIC S9(8) COMP.
				//   05 PTR REDEFINES PTR-ADDR USAGE POINTER.
				if (dataItemGeneric1.getLevelNumber() ==  lvlFieldAtDsp
				&&	dataItemGeneric2.getLevelNumber() ==  lvlFieldAtDsp
				&&  dataItemGeneric2.getUsage() == EnumCobolUsage.USAGE_POINTER
				&&  dataItemGeneric1.getSizeBytes() == 4                                 // 9(8)
				&& 	dataItemGeneric1.getItemType() == EnumDataItemType.COBOL_BINARY      // COMP
				&&  dataItemGeneric2.isRedefinesClause()
				&&  dataItemGeneric2.getRedefinesDataName().equals(dataItemGeneric1.getDataName())) {
					return dataItemGeneric2;
				}
			}
	...
	/* --------------------------------------------------------------------------------------------
	 * Restituisce il parametro Call Using che contiene il pointer che indirizza l'area di Linkage
	 * --------------------------------------------------------------------------------------------
	 * 
	 * Questo metodo viene chiamato nel processo ricorsivo di soluzione istruzioni
	 * dinamiche spreaded, su un programma chiamante
	 * 
 	 * - Si verifica che la call abbia un numero sufficiente di parametri
 	 * - Si verifica l'esistenza del parametro (1-based) in call using )
 	 * 
 	 * L'indicazione del numero di istruzione è in ilpw.callerOrigin.numInstr
 	 * Il parametro potrebbe essere ridefinito e pertanto vengono restituite tutte le possibili aree.
	...
	private ArrayList<InstructionCobolDataItem> getAreaOfUsingParm(LogicWorkProcess ilpw, int numInstrCall, int numParmUsing) {
    	...
    	// Estrazione parametri Call Using Parm1 Parm2 ... Parmn
    	...
    	if (cobolEntryProcDiv.getInstruction() instanceof InstructionCobolProcedure) {
   		...
   		// Call using in programma chiamante con parametri insufficienti X tentare una soluzione
   		if (al_identifierParmUsing.size() < numParmUsing) {
   			return al_areaParmUsing;
		}
   		
		// Porto in output il parametro using di interesse
		numDefParmUsing = al_identifierParmUsing.get(numParmUsing - 1).getNumInstr();
