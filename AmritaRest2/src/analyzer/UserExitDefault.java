...
public class UserExitDefault extends UserExit implements AmritaConstants{
   ...
   /**
	...
	public void executeExit(UserExitInfo uei) {
		...
		switch (uei.getUserExitOperation()) {
				...
				case GET_SYSTEM_OWNER:
					...
					if (uei.getCustomerCode().equals("AMRITA")) {
						executeExitAmrita(uei);
			     	} 					
					break;
					// A fronte di individuazione oggetti da trattare
	...
	/*
	 * Gestione personalizzata per AMRITA di test
	 */
	private void executeExitAmrita(UserExitInfo uei) {
		String sqlOwner = "";
		...
		switch (uei.getUserExitOperation()) {
				...
				// A fronte di individuazione oggetti da trattare
				case GET_SUB_SYSTEM_OWNER:
					if (uei.getCustomerInfo().equals("LOGIC-TEST")) {
						uei.setSubSystem("LO");
					}
					break;
