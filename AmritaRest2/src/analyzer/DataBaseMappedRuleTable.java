 ...
 * Here is an example to map table <code>T001</code> using <code>typeObject</code> java entity field :<pre>
   '@DataBaseMappedTableRule(tabId = "1"; isSystemTab = true; java_key_tab = {"typeObject 1" }'
   </pre>
 * 
...
public @interface DataBaseMappedRuleTable {
	/** Table keys group reference */
	String tabRef();
	/** Table id */
	String tabId();
	/** Table column containing, phisically in the database, the number of the table
	 *  used to realize the LEFT JOIN to get rule table data related to an entity.
	 *   */
	String tabNumCol();
	/** System table, with system and subsystem as a fixed '*' value */
	boolean tabSystem();
	/** Columns bound in the entity as key */
	String[] tabKeyColsBound();
}
