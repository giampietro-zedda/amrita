 ...
 * {@link ProcessAnalyzeSource}, {@link ProcessPgmLevel}, {@link ProcessSystemLevel} etc. <br>
 * <p>
 * Tutte le directories, i path e i file pilota da utilizzare per individuare i sorgenti e il tipo
 * di analisi da effettuare, sono pilotati attraverso le costanti definite nei defaults di sistema,
 * oggetto della classe SystemDefaults e già disponibile.<br>
 * <p>
 * Il processo di analisi viene completamente automatizzato attraverso il file pilota con le direttive
 * di esecuzione, passato come parametro all'interfaccia con l'esterno {@link ExecutionStarter} e localizzato
...
*/
public class ExecutionDispatcher implements AmritaConstants{
	 ...
	 * 
	 */
	public ExecutionDispatcher(UserConfiguration ucfg
						      ,SourceInput siExecutionPilot
						      ) throws ExceptionAmrita {
		
		// Defaults reference ai servizi condivisi
		this.ucfg = ucfg;
		this.mm = ucfg.getMessagesManager();
		this.lf = ucfg.getLoggerFacade();
		this.sm = new SourceManager(ucfg);
		this.rm = new ReflectionManager();
		
		// Da parametri di lancio esterni
    	this.siExecutionPilot = siExecutionPilot;
     	
    	
    	// Allocazioni varie 
      	al_directivesInfoToExec = new ArrayList<ExecutionDirectives>();
	...
	public void processMain() throws ExceptionAmrita, SQLException  {
        ...
        loadSystemDefaultDirSerialized(di);
		...
		if (!isThereDirectiveoStart) {
			lf.writeRow(EnumMessageType.WARNING, "MW0004", null, null); 
			return;
		}
        
		
		///////////////////////////////////////////////////////////////////////////////////////////////
		// (2) Base dati inizializzata in AmritaStartup con creazione pull di connessioni
		//     eventualmente re-inizializzata al login con database utente e nuovo pool di connessioni
		///////////////////////////////////////////////////////////////////////////////////////////////
		...
		try {
			...
			// Se c'è stata qualche exception è gia stata gestita e loggata
    ...
    /*
     * Caricamento directory da default di sistema per pgm/copy/grafi/etc serializzati e sources
     */
	private void loadSystemDefaultDirSerialized(ExecutionDirectives di2) {
	    this.di.dirResources = ucfg.getDirResources();                        // Resource
	    this.di.dirWork = ucfg.getDirWork();                                 	// Working e temporaneo		    
		this.di.dirDatabase = ucfg.getDirDatabase ();                         // Database		              
		this.di.dirJclInput = ucfg.getDirJclSrc();                          // Jcl    in input al processo di analisi (.*)        
		this.di.dirCobolSrcPgm = ucfg.getDirCobolSrcPgm ();         // Pgm    Cobol sorgenti in analisi		  (.*) 
		this.di.dirCobolSrcCopy = ucfg.getDirCobolSrcCopy();        // Copy   Cobol sorgenti in analisi		  (.*)
		this.di.dirCobolObjPgm = ucfg.getDirCobolObjPgm();                  		// Pgm    Cobol codificati e serializzati (.program)			 
		this.di.dirCobolObjCopy = ucfg.getDirCobolObjCopy();                        // Copy   Cobol codificati e serializzati (.copy)	
		this.di.dirJclObj = ucfg.getDirJclObj();                  			        // Jcl    codificati e serializzati (.jclSource, .jclInclude, .jclProc)		 
		this.di.dirSqlScript = ucfg.getDirSqlScript();                  		// Script Sql codificati e serializzati   (.scriptSql)			 
		this.di.dirCobolGraph = ucfg.getDirCobolGraph();                		// Grafi  Cobol codificati e serializzati (.graph)	
		this.di.dirLog = ucfg.getDirLog();                                 	// Log
		this.di.dirOutput = ucfg.getDirOutput();                              // Output per funzioni 
	...
	private void execFunctionsAndProcesses() throws Exception {
		...
		///////////////////////////////////////////////////////////////////////////////////////////
		// (1) Esecuzione funzioni/processi che devono essere eseguite ognuna in un thread separato 
		///////////////////////////////////////////////////////////////////////////////////////////
			...
			if (!execAsThread  && !diFunctionProcess.isToExecAsThread) {
				...
				try {
						...
						try {
							...
							rm.invokeMethod(ObjFunctionToExec, "exec", null, null);
			        	    ...
			        	    strMsg[1] = "exec";
						    lf.writeRow(EnumMessageType.ERROR_INTERNAL, "ET0006", strMsg, excp);
						    throw excp;
						}
	 ...
	 * 
	 */
	private void extractInfoDirective(EnumDirectivesExecution enProcessOrFunction, String rowPilot) {
		...
		switch (enProcessOrFunction){
	     ...
	     case DIR_PILOT_AND_FILTER:              			// Pilot  sources e processi e filtri     (.pilot)	    	 
			if (isThereDirectiveoStart) {
				di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_PILOT_AND_FILTER, rowPilot);
			}
			return;

	     case DIR_WORK:                                		// Working e temporaneo		    
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_WORK, rowPilot);
				}
				return;

		 case DIR_LOG:                                 		// Log
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_LOG, rowPilot);
				}
				return;

		 case DIR_OUTPUT:                              		// Output per funzioni 
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_OUTPUT, rowPilot);
				}
				return;

		 case DIR_RESOURCES:                        			// Resource
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_RESOURCES, rowPilot);
				}
				return;

		 case DIR_DATABASE:                          		// Database		              
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_DATABASE, rowPilot);
				}
				return;

		 case DIR_COBOL_SRC_PGM_INPUT:          				// Pgm    Cobol sorgenti in analisi		  (.*) 
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_COBOL_SRC_PGM_INPUT, rowPilot);
				}
				return;

		 case DIR_COBOL_SRC_COPY_INPUT:        				// Copy   Cobol sorgenti in analisi		  (.*)
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_COBOL_SRC_COPY_INPUT, rowPilot);
				}
				return;

		 case DIR_COBOL_PGM:                  				// Pgm    Cobol codificati e serializzati (.program)			 
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_COBOL_PGM, rowPilot);
				}
				return;

		 case DIR_COBOL_COPY:                        		// Copy   Cobol codificati e serializzati (.copy)	
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_COBOL_COPY, rowPilot);
				}
				return;
		 ...
		 case DIR_SQL_SCRIPT:                  				// Script Sql codificati e serializzati   (.scriptSql)			 
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_SQL_SCRIPT, rowPilot);
				}
				return;

		 case DIR_JCL_INPUT:                          		// Jcl    in input al processo di analisi (.*)        
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_JCL_INPUT, rowPilot);
				}
				return;

		 case DIR_JCL:                 			        	// Jcl    codificati e serializzati (.jclSource, .jclInclude, .jclProc)		 
				if (isThereDirectiveoStart) {
					di.dirPilot = getDirFromRowpilot(EnumDirectivesExecution.DIR_JCL, rowPilot);
				}
				return;
		...
		case METRICS_SQUALE_VIOLATIONS_SCENARIO:
			...
			if (ss == null) {
				arParm = new String[1];
				arParm[0] = rowPilot.trim();
				lf.writeRow(EnumMessageType.ERROR_INPUT, "ET0004", arParm, null); 
				return;
			}
		...
		case LIBRARY:
			...
			if (!sourceInputInfo.isExists()) {
				arParm = new String[1];
				arParm[0] = ss._word(2);
				lf.writeRow(EnumMessageType.ERROR_INPUT, "ET0002", arParm, null); 
				return;
			}
		...
		// PROCESS_ANALYZE_SOURCES & PROCESS_PGM_LEVEL
			...
			// Creazione grafo di programma abilitata
	...
	*/
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////
		 ...
		 * 
		 */
		private ArrayList<ArrayList<ExecutionDirectives>> splitInfoExecutionUntit() {
		...
		private Object newFunctionInstance(String functionClassNameComplete
				...
				ObjFunctionToExec = rm.newInstance(functionClassNameComplete, ar_Class, ar_Object);
				...
				if (ObjFunctionToExec == null) {
				    ...
				    lf.writeRow(EnumMessageType.ERROR_INTERNAL, "ET0005", strMsg, excp);
				    throw excp;
				}
		...
		private void loggingStartProcessFunction(ArrayList<ExecutionDirectives> al_execUnitDirectives, ExecutionDirectives curProcessFunctionInfo) {
			...
			lf.writeRow(EnumMessageType.INFORMATION, "MI0016", arParm, null); 
		}
		...
		private void loggingEndProcessFunction(ArrayList<ExecutionDirectives> al_execUnitInfo, ExecutionDirectives curProcessFunctionInfo) {
			...
			// Durata in ms complessiva e media per oggetto disponibili da webService
			this.ucfg.setExecMsAllElapsed(curProcessFunctionInfo.execMsAllElapsed);
			this.ucfg.setExecMsAvg(timeMsMean);
            						
			// Con o senza errori
			if (isExecutionWithErrors(al_execUnitInfo)) {
				excecutionState = mm.getMessage(ucfg.getCurModule(), EnumMessageType.INFORMATION, "TC0002");
			} else {
				excecutionState = mm.getMessage(ucfg.getCurModule(), EnumMessageType.INFORMATION, "TC0001");
			}
			arParm[3] = excecutionState;
			
			// Processo/funzione terminato in x ms
			lf.writeRow(EnumMessageType.INFORMATION, "MI0017", arParm, null); 
		}
