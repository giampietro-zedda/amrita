...
*/
public class _ProcessAnalyzeSource extends ExecutionShared implements Runnable, AmritaConstants{
	 ...
	 * 
	 */
	public _ProcessAnalyzeSource(UserConfiguration sd, ArrayList<ExecutionDirectives> al_di) {
    ...
    */
	...
	public void execAnalyzeSources() throws Exception {
	    ...
	    for (InnerDescriptorSource infoObjectToAnalyze : al_ObjectToAnalize) {
            ...
            this.si.setSubSystemOwner(infoObjectToAnalyze.subSys);
	...
	private void activationSpecificAnalyzer(InnerDescriptorSource objectToAnalyze, SourceInput si, int cntSource, int totSource) throws Exception {
    	...
    	switch (objectToAnalyze.objectType) {
				...
				case OBJECT_PGM_COBOL: 
					...
					this.di.systemInput = userExitInfoPgm.getSystem();
				    this.di.subSystemInput = userExitInfoPgm.getSubSystem();
